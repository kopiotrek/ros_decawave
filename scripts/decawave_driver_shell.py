...
class DecawaveDriver(object):
    ...
    def __init__(self):
        rclpy.init_node('decawave_driver', anonymous=False)
        # Getting Serial Parameters
        self.port_ = rclpy.get_param('port', '/dev/ttyACM0')
        self.baudrate_ = int(rclpy.get_param('baudrate', '115200'))
        self.tf2_ros_publisher_ = rclpy.get_param('tf2_ros_publisher', 'True')
        self.rate_ = int(rclpy.get_param('rate', '10'))
        # Initiate Serial
        self.ser = serial.Serial(self.port_, self.baudrate_, timeout=0.1)
        rclpy.loginfo("\33[96mConnected to %s at %i\33[0m",
                      self.ser.portstr, self.baudrate_)
        self.get_uart_mode()
    ...
    def get_uart_mode(self):
        ...
        if cc == '@\x01\x01':  # GENERIC MODE
            rclpy.loginfo(
                "\33[96mDevice is on GENERIC MODE!  It must to be changed to SHELL MODE!\33[0m")
            self.mode_ = "GENERIC"
        else:  # SHELL MODE
            rclpy.loginfo("\33[96mDevice is on SHELL MODE! Ok!\33[0m")
            self.mode_ = "SHELL"
    ...
    def switch_uart_mode(self):
        ...
        if self.mode_ == "GENERIC":
            rclpy.loginfo("\33[96mChanging UART mode to SHELL MODE...\33[0m")
            self.ser.write(b'\r\r')  # Go to Shell Mode
            while(self.ser.inWaiting() == 0):
                pass
            time.sleep(1.0)
            self.ser.flushInput()
            rclpy.loginfo("\33[96m%s\33[0m",
                          self.ser.readline().replace('\n', ''))
        elif self.mode_ == "UNKNOWN":
            rclpy.logerr(
                "%s", "Unknown Mode Detected! Please reset the device and try again!")
    ...
    def get_tag_version(self):
        ...
        rclpy.loginfo("\33[96m--------------------------------\33[0m")
        rclpy.loginfo("\33[96mFirmware Version:0x" +
                      format(data_[5], '04X')+"\33[0m")
        rclpy.loginfo("\33[96mConfiguration Version:0x" +
                      format(data_[8], '04X')+"\33[0m")
        rclpy.loginfo("\33[96mHardware Version:0x" +
    ...
    #        print("%s", data_)
    #    if data_[5] == 3:
    #        rclpy.loginfo("\33[96mTag is CONNECTED to a UWB network and LOCATION data are READY!\33[0m")
    #    elif data_[5] == 2:
    #        rclpy.logwarn("Tag is CONNECTED to a UWB network but LOCATION data are NOT READY!")
    #    elif data_[5] == 1:
    #        rclpy.logwarn("Tag is NOT CONNECTED to a UWB network but LOCATION data are READY!")
    #    elif data_[5] == 0:
    #        rclpy.logwarn("Tag is NOT CONNECTED to a UWB network and LOCATION data are NOT READY!")
    ...
    def get_tag_acc(self):
        ...
        while not 'acc' in cc:
            cc = self.ser.readline()
            if rclpy.Time.now() - t > rclpy.Duration(0.5):
                rclpy.logwarn("Could not get accel data!")
        cc = cc.replace('\r\n', '').replace('acc: ', '').split(',')
        if len(cc) == 3:
            ...
            self.accel.header.stamp = rclpy.Time.now()
    ...
    def get_tag_location(self):
        ...
        while not 'DIST' in cc:
            ...
            print(cc)
            if rclpy.Time.now() - t > rclpy.Duration(0.5):
                rclpy.logwarn("Could not get tag data!")
                self.ser.flushInput()
                self.ser.write(b'\r')  # Test Mode
        #cc = ''
        #t = rclpy.Time.now()
    ...
    def tf2_ros_callback(self, timer):
        if self.tf2_ros_publisher_ == 'True':
            self.br.sendTransform((self.tag.x, self.tag.y, self.tag.z),
                                  tf2_ros.transformations.quaternion_from_euler(
                                      0, 0, 0),
                                  rclpy.Time.now(),
                                  "tag",
                                  "world")
    ...
    def run(self):
        ...
        rclpy.loginfo("\33[96mInitiating Driver...\33[0m")
        self.tag_pub_ = rclpy.Publisher('pose', Tag, queue_size=1)
        self.anchors_pub_ = rclpy.Publisher(
            'status', AnchorArray, queue_size=1)
        self.acc_pub_ = rclpy.Publisher('accelerometer', Acc, queue_size=1)
        self.timer = rclpy.Timer(rclpy.Duration(0.2), self.tf2_ros_callback)
...
if __name__ == '__main__':
    try:
        ...
        rclpy.loginfo("[Decawave Driver]: Closed!")
